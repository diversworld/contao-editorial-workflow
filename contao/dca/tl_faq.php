...
if (isset($GLOBALS['TL_DCA']['tl_faq']['palettes']) && is_array($GLOBALS['TL_DCA']['tl_faq']['palettes'])) {
    foreach ($GLOBALS['TL_DCA']['tl_faq']['palettes'] as $name => $palette) {
        ...
        if (str_contains($palette, 'publish_legend')) {
            ...
            $GLOBALS['TL_DCA']['tl_faq']['palettes'][$name] .= ';{workflow_legend},workflow_status,workflow_comment';
            continue;
        }

        $pm->applyToPalette($name, 'tl_faq');
    }
}
